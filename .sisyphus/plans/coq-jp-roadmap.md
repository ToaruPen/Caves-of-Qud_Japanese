...
- GitHub Actions CI ゲート（lint・test・build が PR マージの必須条件）
- 最厳格Linter設定: Python は Ruff `select = ["ALL"]` + McCabe C ≤ 10 + complexity ≤ 10, C# は `TreatWarningsAsErrors` + Nullable enable + `EnforceCodeStyleInBuild` + `EnableNETAnalyzers` + `AnalysisLevel=latest-all` + SonarAnalyzer.CSharp（マジックナンバー S109、エラー握りつぶし S2737/S2221、ハードコード資格情報 S2068/S6418 を検出）+ Custom Roslyn Analyzer（Harmonyパッチtry-catch強制 QJ001、フォールバック前ログ強制 QJ002、ログプレフィックス強制 QJ003）
- Fail-fast エラー処理: 初期化時（`Init`, 翻訳ロード）は例外をスローし即座に失敗させる。ランタイムパッチ（Prefix/Postfix内）はゲームクラッシュを避けるため例外を投げないが、`Trace.TraceError` で目立つログを出す（fail-loud）。サイレントな `return;` / `return null;` で障害を隠蔽しない

### Must NOT Have (Guardrails)
- レガシーのC#/Pythonコードのコピー＆ペースト（参考程度の閲覧は可、コード流用は不可）
- lintなし・型チェックなしのコード（C#は`TreatWarningsAsErrors` + Nullable enable、Pythonは Ruff `select = ["ALL"]` + McCabe C ≤ 10 + complexity ≤ 10）
- CI未通過のPRマージ（GitHub Actions の lint・test・build 全ジョブ pass が必須）
...
- サイレントエラー握り潰し（`catch (Exception) { Trace.TraceWarning(...); }` で例外を飲み込む、`if (x is null) return;` で想定外の null を無視する、`errors="ignore"` でデコードエラーを隠す等）— 障害の発見を遅延させるため禁止
...
> 1. **RED**: 実ゲーム DLL で `TargetMethod` / シグネチャ / 安全な static メソッドの検証を書く。必要なら DummyTarget でも期待する翻訳結果を先に固定する（この時点で失敗する）
> 2. **GREEN**: テストを通す最小限のパッチ実装を書く
> 3. **REFACTOR**: コードを整理し、色制御文字保全・エッジケースのテストを追加
>
> **TDDの適用範囲**:
> - L1 (Pure Logic) テスト: Grammar変換、色コード保全、辞書検索 — **必ずテストファースト**
> - L2 (Harmony Integration) テスト: game-DLL-assisted な target 解決 / シグネチャ検証 + DummyTarget または安全な実メソッドによるパッチ適用 — **必ずテストファースト**
> - L3 (Game Smoke Test): ゲーム内動作確認 — テストファーストは不可（手動QA）
>
> **TDDの利点（本プロジェクト固有）**:
> - 実ゲーム DLL に対する自動検証を先に置くことで、パッチの hook point が現行ゲーム 2.0.4 と一致していることを保証
> - Pure関数（~85%）のテストを先に書くことで、Unity環境なしで翻訳ロジックの正確性を担保
> - DummyTarget は実 DLL で安全に再現しづらいパッチ本文の補助検証として使える
...
| **L2: Harmony Integration** | パッチ適用, 実 DLL 上のフック解決, DummyTarget/安全な実メソッドでの Prefix/Postfix 動作 | NUnit + HarmonyLib | ❌ なし | `dotnet test --filter TestCategory=L2` |
...
### DummyTarget 設計原則
- まず実ゲーム DLL 上で `TargetMethod` / シグネチャ / 安全な static メソッドを検証する
- 実ゲームの型をテスト内で直接 instantiate しない
- メソッドシグネチャのみを再現するDummyクラスは、実 DLL で安全に再現しづらい本文ロジックの補助検証として使う
- DummyTargetのシグネチャはILSpy解析結果と実 DLL の両方と照合して正確に再現する
...
Wave 8 (After Wave 7 — polish, 4 parallel):
├── Task 18: フルゲームプレイテスト (depends: 12, 13, 14, 16, 17) [unspecified-high]
├── Task 19: ドキュメント整備 (depends: 8, 15) [writing]
├── Task 21: Roslyn Analyzer強化 (depends: 1, 7) [unspecified-high]
└── Task 22: Custom Roslyn Analyzer (depends: 1, 21) [deep]

Wave 9 (After Wave 8 — release):
└── Task 20: GitHubリリース準備 (depends: 18, 19, 21, 22) [quick]
...
| 7 | 1 | T17 → ultrabrain |
| 8 | 4 | T18 → unspecified-high, T19 → writing, T21 → unspecified-high, T22 → deep |
| 9 | 1 | T20 → quick |
...
- [x] 1. プロジェクトスキャフォールディング
  ...
  **What to do**:
  ...
  - `README.md` 骨子作成
  - Python環境設定（`pyproject.toml`、Ruff 最厳格設定）:
    - `select = ["ALL"]`（全ルール有効化、必要最小限の `ignore` のみ許可）
    - `[tool.ruff.lint.mccabe] max-complexity = 10`（McCabe C ≤ 10）
    - `[tool.ruff.lint.pylint] max-complexity = 10`（Pylint complexity ≤ 10）
    - `[tool.ruff.lint.pydocstyle] convention = "google"`
    - `target-version = "py312"` 以上
  - C# プロジェクト作成（`QudJP.csproj` — Task 0 PoCで決定したTarget Framework, GameDir設定をmacOS対応に）:
    - `<Nullable>enable</Nullable>`
    - `<TreatWarningsAsErrors>true</TreatWarningsAsErrors>`
    - `<EnforceCodeStyleInBuild>true</EnforceCodeStyleInBuild>`
    - `.editorconfig` にC#コードスタイルルール（severity = error）
  - `.github/workflows/ci.yml` — GitHub Actions CI ゲート:
    - トリガー: `push` (main) + `pull_request`
    - ジョブ: `dotnet build -warnaserror` → `dotnet test` → `ruff check scripts/` → `pytest scripts/tests/` → `python scripts/check_encoding.py Mods/QudJP/Localization/`
    - 全ジョブ pass が PR マージの必須条件（Branch protection rule で `required status checks` に設定）
  - **AGENTS.md / CLAUDE.md 配置** (OpenAI Codex + Anthropic Claude Code ベストプラクティス準拠):
    - ルート `AGENTS.md` — プロジェクト概要、技術スタック、ビルド/テストコマンド、全体規約
    - ルート `CLAUDE.md` → `AGENTS.md` へのシンボリックリンク (`ln -s AGENTS.md CLAUDE.md`)
    - `Mods/QudJP/Assemblies/AGENTS.md` — C#規約 (Nullable enable, Harmony パッチパターン, DummyTarget, テスト層分離)
    - `scripts/AGENTS.md` — Python規約 (Ruff, 型ヒント, スクリプト命名規則)
    - `Mods/QudJP/Localization/AGENTS.md` — XML規約 (UTF-8 BOM無し, Load="Merge", 色コード3形式の保全)
    - 各ファイルは200行以内を目標（Anthropic推奨）
    - カスケード発見に対応: サブディレクトリのAGENTS.mdはそのディレクトリ固有のルールのみ記載
  ...
  **Acceptance Criteria**:
  ...
  - [ ] ルート `AGENTS.md` が存在し、プロジェクト概要・技術スタック・ビルドコマンドを含む
  - [ ] ルート `CLAUDE.md` が `AGENTS.md` へのシンボリックリンクである (`readlink CLAUDE.md` → `AGENTS.md`)
  - [ ] `Mods/QudJP/Assemblies/AGENTS.md`, `scripts/AGENTS.md`, `Mods/QudJP/Localization/AGENTS.md` が存在
  - [ ] 各AGENTS.mdが200行以内
  - [ ] `.github/workflows/ci.yml` が存在し、lint・test・build 全ジョブを含む
  - [ ] Ruff 設定が `select = ["ALL"]` かつ `max-complexity = 10`（McCabe C, Pylint complexity 両方）
  - [ ] C# プロジェクトが `TreatWarningsAsErrors` + `Nullable enable` + `EnforceCodeStyleInBuild` 有効
  ...
  **QA Scenarios**:
  ...
  Scenario: AGENTS.md / CLAUDE.md 配置の検証
    Tool: Bash
    Steps:
      1. test -f AGENTS.md && echo "root AGENTS.md exists"
      2. readlink CLAUDE.md で AGENTS.md へのシンボリックリンクを確認
      3. test -f Mods/QudJP/Assemblies/AGENTS.md && test -f scripts/AGENTS.md && test -f Mods/QudJP/Localization/AGENTS.md
      4. wc -l AGENTS.md Mods/QudJP/Assemblies/AGENTS.md scripts/AGENTS.md Mods/QudJP/Localization/AGENTS.md で各ファイルが200行以内か確認
    Expected Result: 全AGENTS.mdが存在、CLAUDE.mdがシンボリックリンク、各ファイル200行以内
    Evidence: .sisyphus/evidence/task-1-agents-md.txt

  Scenario: CI ワークフロー & Linter 厳格設定の検証
    Tool: Bash
    Steps:
      1. test -f .github/workflows/ci.yml && echo "CI workflow exists"
      2. grep -c "ruff check\|dotnet test\|pytest\|dotnet build" .github/workflows/ci.yml で CI ジョブの存在確認（4以上）
      3. grep 'select = \["ALL"\]' pyproject.toml で Ruff 最厳格設定の確認
      4. grep "max-complexity = 10" pyproject.toml で complexity 設定の確認（2箇所: mccabe + pylint）
      5. grep "TreatWarningsAsErrors" Mods/QudJP/Assemblies/QudJP.csproj で C# 厳格設定確認
      6. grep "EnforceCodeStyleInBuild" Mods/QudJP/Assemblies/QudJP.csproj で C# コードスタイル強制確認
    Expected Result: CI ワークフロー存在、Ruff ALL 設定、complexity 10（C + complexity 両方）、C# 厳格設定全項目
    Failure Indicators: CI ファイル不在、Ruff 設定が ALL でない、complexity 上限が 10 超、TreatWarningsAsErrors 未設定
    Evidence: .sisyphus/evidence/task-1-ci-linter.txt
  ```
...
- [x] 3. レガシーXML監査 & 移行計画
  ...
  **What to do**:
  ...
  - **翻訳データ完全性チェック** (レガシー翻訳 vs 現行ゲームデータの照合):
    - 現行ゲームの `StreamingAssets/Base/` 配下の全XMLからBlueprint ID / Conversation ID / Option ID等を抽出
    - レガシー翻訳XMLから翻訳済みIDを抽出
    - 差分を算出: 「ゲームに存在するがレガシー翻訳にないエントリ」= 未翻訳リスト
    - 逆差分も算出: 「レガシー翻訳にあるがゲームに存在しないエントリ」= 削除済み/リネーム済みリスト
    - カテゴリ別カバレッジ率を算出 (Creatures, Items, Conversations, Options, Skills等)
    - 結果を `docs/translation-coverage-report.md` に出力
    - `migration-report.json` にカバレッジ統計を追加
  ...
  **Acceptance Criteria**:
  ...
  - [ ] `docs/translation-coverage-report.md` が存在し、カテゴリ別カバレッジ率を含む
  - [ ] 未翻訳エントリ数と削除済みエントリ数が定量的に把握されている
  - [ ] `migration-report.json` にカバレッジ統計セクションが含まれている
  ...
  **QA Scenarios**:
  ...
  Scenario: 翻訳データ完全性チェックの検証
    Tool: Bash
    Steps:
      1. test -f docs/translation-coverage-report.md && echo "coverage report exists"
      2. grep -c "カバレッジ" docs/translation-coverage-report.md でカバレッジ情報の存在確認
      3. jq '.coverage' migration-report.json でカバレッジ統計セクションの存在確認
      4. jq '.coverage.categories | keys' migration-report.json でカテゴリ一覧確認
    Expected Result: カバレッジレポートが存在し、カテゴリ別の翻訳率・未翻訳数が記載されている
    Evidence: .sisyphus/evidence/task-3-coverage-check.txt
  ```
...
- [x] 11. Grammar系パッチ (冠詞除去, 複数形中和等) + L1テスト
...
- [x] 21. Roslyn Analyzer 強化 (EnableNETAnalyzers + SonarAnalyzer.CSharp)

  **What to do**:
  - **背景**: `TreatWarningsAsErrors=true` は警告をエラーに変換する「執行機構」だが、net48 では `EnableNETAnalyzers` がデフォルト OFF のため CA ルールが無効 = 「弾なしの銃」状態。加えて CA ルール単体ではマジックナンバー・ハードコード資格情報・エラー握りつぶしパターンのカバレッジが不十分（約20-30%）。SonarAnalyzer.CSharp を追加して約80%のカバレッジに引き上げる。
  - **Step 1: .NET Analyzer 有効化**
    - `QudJP.csproj` に `<EnableNETAnalyzers>true</EnableNETAnalyzers>` + `<AnalysisLevel>latest-all</AnalysisLevel>` を追加
    - `QudJP.Tests.csproj` に `<AnalysisLevel>latest-all</AnalysisLevel>` を追加（net10.0 は EnableNETAnalyzers がデフォルト ON だが AnalysisLevel を明示）
  - **Step 2: SonarAnalyzer.CSharp NuGet 追加**
    - 両 csproj に `<PackageReference Include="SonarAnalyzer.CSharp" Version="..." PrivateAssets="all" />` を追加
    - Analyzer-only パッケージのため runtime 依存なし、net48 互換
  - **Step 3: ビルドして新規警告を全件収集**
    - `dotnet build` を実行し、CA/S ルールの全警告を一覧化
  - **Step 4: トリアージ — 3分類**
    - **修正する（実問題）**:
      - マジックナンバー `8`, `19`, `4`, `9`, `14` in `PopupTranslationPatch.cs` → 名前付き定数に変換
      - サイレント `??` フォールバック in `LocalizationAssetResolver.cs:53,56` と `MainMenuLocalizationPatch.cs:27` → `Trace.TraceWarning` を追加
      - `ColorCodePreserver.cs` のマジックナンバー `2` → `ColorTokenLength` 等の名前付き定数に変換
    - **プロジェクト固有の抑制（正当な理由あり）**:
      - Harmony パッチの `AccessTools.Method()` 文字列引数 → ゲーム API 参照のため不可避、`.editorconfig` で抑制
      - `Interlocked.Exchange(..., 1)` のセンチネル値 → ブーリアンの代替表現、抑制
      - テストプロジェクト固有の抑制（magic values in test data 等）
    - **今後の回帰防止として維持する（新コードに適用）**:
      - CA1031 (汎用 Exception catch 禁止)、S2737 (catch は rethrow 以外を行うべき)、S109 (マジックナンバー禁止) 等
  - **Step 5: `.editorconfig` に抑制ルール追加**
    - プロジェクト固有の正当な抑制を `dotnet_diagnostic.XXXX.severity = none/suggestion` で設定
  - **Step 6: ビルド確認 — ゼロ警告**
    - `dotnet build` が 0 warnings で成功すること
    - `dotnet test` が全テストパスすること

  **Must NOT do**:
  - 警告を `#pragma warning disable` で個別抑制しない（`.editorconfig` で一括管理）
  - 全 S/CA ルールを無差別に抑制しない（個々の抑制に理由コメントを `.editorconfig` に記載）
  - 実問題（マジックナンバー、サイレントフォールバック）を抑制で済ませない — 必ずコード修正する
  ...
  **References**:
  - 調査結果: CA ルールはマジックナンバー検出なし、シークレット検出なし、サイレントフォールバック検出なし
  - SonarAnalyzer.CSharp: S109 (マジックナンバー)、S2737 (catch rethrow)、S2221 (汎用 Exception)、S2068 (ハードコード資格情報)、S6418 (シークレット)
  - 現行の問題箇所: `PopupTranslationPatch.cs:17,23,60,61,63,65`、`LocalizationAssetResolver.cs:53,56`、`MainMenuLocalizationPatch.cs:27`、`ColorCodePreserver.cs` 全体

  **Acceptance Criteria**:
  - [ ] `QudJP.csproj` に `EnableNETAnalyzers=true` + `AnalysisLevel=latest-all` が設定されている
  - [ ] `SonarAnalyzer.CSharp` が両 csproj に追加されている
  - [ ] `dotnet build` が 0 warnings で成功する
  - [ ] `dotnet test` が全テストパスする
  - [ ] `PopupTranslationPatch.cs` のマジックナンバーが名前付き定数に変換されている
  - [ ] サイレント `??` フォールバックにログが追加されている
  - [ ] `.editorconfig` にプロジェクト固有の抑制ルールが理由付きで記載されている
  - [ ] `#pragma warning disable` がソースコード内に存在しない

  **QA Scenarios**:
  ```
  Scenario: Analyzer 有効化とゼロ警告ビルド
    Tool: Bash
    Steps:
      1. grep "EnableNETAnalyzers" Mods/QudJP/Assemblies/QudJP.csproj でアナライザー有効確認
      2. grep "SonarAnalyzer" Mods/QudJP/Assemblies/QudJP.csproj でパッケージ追加確認
      3. dotnet build Mods/QudJP/Assemblies/QudJP.csproj で 0 warnings ビルド
      4. dotnet test Mods/QudJP/Assemblies/QudJP.Tests/QudJP.Tests.csproj で全テストパス
    Expected Result: Analyzer 設定あり、ビルド成功(0 warnings)、全テストパス
    Evidence: .sisyphus/evidence/task-21-analyzer-build.txt

  Scenario: マジックナンバー修正の検証
    Tool: Bash
    Steps:
      1. grep -n "parameterCount:" Mods/QudJP/Assemblies/src/Patches/PopupTranslationPatch.cs で名前付き定数使用を確認
      2. grep -n "const.*int" Mods/QudJP/Assemblies/src/Patches/PopupTranslationPatch.cs で定数定義確認
    Expected Result: 数値リテラル直書きではなく名前付き定数を使用
    Evidence: .sisyphus/evidence/task-21-magic-numbers.txt

  Scenario: サイレントフォールバック修正の検証
    Tool: Bash
    Steps:
      1. grep -n "Trace.*fallback\|Trace.*resolver\|Trace.*directory" Mods/QudJP/Assemblies/src/LocalizationAssetResolver.cs でログ追加確認
      2. grep -c "#pragma warning disable" Mods/QudJP/Assemblies/src/**/*.cs で pragma 不使用確認（0件期待）
    Expected Result: フォールバック箇所にログあり、pragma 0件
    Evidence: .sisyphus/evidence/task-21-silent-fallback.txt
  ```

  **Commit**: YES
  - Message: `chore(lint): enable .NET analyzers + SonarAnalyzer.CSharp and fix warnings`

- [x] 22. Custom Roslyn Analyzer (プロジェクト固有ルールのビルド時強制)

  **What to do**:
  - QJ001: Harmony パッチ本体 (Prefix/Postfix) に try-catch 強制
  - QJ002: フォールバック値 (`??` on method call) の前にログ強制
  - QJ003: `Trace.TraceError`/`Trace.TraceWarning` メッセージに `QudJP:` プレフィックス強制
  - netstandard2.0 Analyzer プロジェクト + net10.0 テストプロジェクト
