...
def _dedupe_key(entry: LogEntry) -> tuple[str, ...]:
    ...
    if entry.kind == LogEntryKind.FINAL_OUTPUT_PROBE:
        return (
            ...
            entry.markup_semantic_status or "",
            entry.markup_semantic_flags or "",
...
def _entry_with_structured_fields(
    ...
    return LogEntry(
        ...
        markup_semantic_status=structured("markup_semantic_status", entry.markup_semantic_status),
        markup_semantic_flags=structured("markup_semantic_flags", entry.markup_semantic_flags),
