...
public sealed class CombatAndLogMessageQueuePatchTests
{
    ...
    [TestCase("you are crippled for 5 turns!", "you are crippled for 5 turns!")]
    [TestCase("You are crippled!", "You are crippled!")]
    [TestCase("You are crippled for", "You are crippled for")]
    public void CrippleApply_DoesNotTranslate_WhenMessageDoesNotMatchGuardPredicate_WhenPatched(string source, string expected)
    {
        WritePatternDictionary(
            ("^You are crippled for (.+?)!$", "{t0}のあいだ手足が不自由になった！"));
        WriteLeafDictionary(("5 turns", "5ターン"));
    ...
    [Test]
    public void CrippleApply_FinalizerDecrementsDepthToZero_AfterOwnerCallCompletes()
    {
        WritePatternDictionary(
            ("^You are crippled for (.+?)!$", "{t0}のあいだ手足が不自由になった！"));
        WriteLeafDictionary(("3 turns", "3ターン"));
        ...
        try
        {
            ...
            var target = new DummyCrippleApplyTarget
            {
                MessageToSend = "You are crippled for 3 turns!",
            };
            target.Apply(new DummyGameObject());

            // After the owner call completes, a queue-only message should NOT be translated
            // (depth is back to 0, so TryTranslateQueuedMessage returns false immediately)
            DummyMessageQueue.Reset();
            DummyMessageQueue.AddPlayerMessage("You are crippled for 3 turns!", null, Capitalize: false);

            Assert.That(DummyMessageQueue.LastMessage, Is.EqualTo("You are crippled for 3 turns!"),
                "Queue-only message after owner scope closes should not be translated (depth=0)");
        }
        finally
        {
            harmony.UnpatchAll(harmonyId);
    ...
    [TestCase("You gain 75XP!", "You gain 75XP!")]
    [TestCase("you gain 75 XP!", "you gain 75 XP!")]
    [TestCase("You gain 75 XP now!", "You gain 75 XP now!")]
    [TestCase("The creature gains 75 XP!", "The creature gains 75 XP!")]
    public void ExperienceAwardXp_DoesNotTranslate_WhenMessageDoesNotMatchGuardPredicate_WhenPatched(string source, string expected)
    {
        UseRepositoryPatternDictionary();
    ...
    [Test]
    public void ExperienceAwardXp_FinalizerDecrementsDepthToZero_AfterOwnerCallCompletes()
    {
        UseRepositoryPatternDictionary();
        ...
        try
        {
            ...
            var target = new DummyExperienceTarget { MessageToSend = "You gain {{C|75}} XP!" };
            target.HandleEvent(new DummyAwardXPEvent());

            // After owner scope closes, queue-only traffic should NOT be translated
            DummyMessageQueue.Reset();
            DummyMessageQueue.AddPlayerMessage("You gain 50 XP!", null, Capitalize: false);

            Assert.That(DummyMessageQueue.LastMessage, Is.EqualTo("You gain 50 XP!"),
                "Queue-only message after owner scope closes should not be translated (depth=0)");
        }
        finally
        {
