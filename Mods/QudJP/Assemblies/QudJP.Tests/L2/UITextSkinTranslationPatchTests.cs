...
public sealed class UITextSkinTranslationPatchTests
{
    ...
    [Test]
    public void MissileWeaponAreaPostfix_WithBothNullParameters_DoesNotThrow()
    {
        WriteDictionaryFile(
            "ui-missile-weapon-area.ja.json",
            ("fire", "射撃"),
        ...
        Assert.DoesNotThrow(() => MissileWeaponAreaTranslationPatch.Postfix(null, null));
    }

    [Test]
    public void MissileWeaponAreaPostfix_WithNullFireAndValidReload_TranslatesOnlyReload()
    {
        WriteDictionaryFile(
            "ui-missile-weapon-area.ja.json",
            ("fire", "射撃"),
        ...
        MissileWeaponAreaTranslationPatch.Postfix(null, reload);

        Assert.That(reload.Text, Is.EqualTo("{{W|[R]}} リロード"));
    }

    [Test]
    public void MissileWeaponAreaPostfix_WithNullReloadAndValidFire_TranslatesOnlyFire()
    {
        WriteDictionaryFile(
            "ui-missile-weapon-area.ja.json",
            ("fire", "射撃"),
        ...
        MissileWeaponAreaTranslationPatch.Postfix(fire, null);

        Assert.That(fire.Text, Is.EqualTo("{{W|[F]}} 射撃"));
    }

    [Test]
    public void TranslatePreservingColors_PickTarget_CommandBarWithFireMissileWeaponSegmentPassesThrough()
    {
        WriteDictionaryFile(
            "ui-default.ja.json",
            ("lock", "ロック"));
        ...
        // "Fire Missile Weapon" is an owner-route token and should pass through unchanged in any casing
        var source = "{{W|space}}-select | unlock ({{hotkey|F1}}) | Fire Missile Weapon";
        var translated = UITextSkinTranslationPatch.TranslatePreservingColors(
            source,
            nameof(UITextSkinTranslationPatch));

        Assert.That(translated, Is.EqualTo("{{W|space}}-選択 | ロック解除 ({{hotkey|F1}}) | Fire Missile Weapon"));
    }

    [Test]
    public void TranslatePreservingColors_PickTarget_CommandBarWithFireMissileWeaponCaseInsensitivePassesThrough()
    {
        WriteDictionaryFile(
            "ui-pick-target.ja.json",
            ("select", "選択"));

        // IsOwnerRouteCommandBarToken uses OrdinalIgnoreCase, so "fire missile weapon" also passes through
        var source = "{{W|space}}-select | fire missile weapon";
        var translated = UITextSkinTranslationPatch.TranslatePreservingColors(
            source,
            nameof(UITextSkinTranslationPatch));

        Assert.That(translated, Is.EqualTo("{{W|space}}-選択 | fire missile weapon"));
    }

    [Test]
    public void TranslatePreservingColors_PickTarget_CommandBarWithReloadSegmentPassesThrough()
    {
        WriteDictionaryFile(
            "ui-pick-target.ja.json",
            ("select", "選択"));

        // "Reload" is an owner-route token and should pass through unchanged in any casing
        var source = "{{W|space}}-select | Reload";
        var translated = UITextSkinTranslationPatch.TranslatePreservingColors(
            source,
            nameof(UITextSkinTranslationPatch));

        Assert.That(translated, Is.EqualTo("{{W|space}}-選択 | Reload"));
    }
