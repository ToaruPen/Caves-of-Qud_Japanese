...
| 層 | テストファイル数 | テストメソッド | `[TestCase]` 件 | invocation 計 |
|---|---:|---:|---:|---:|
| L1 | 52 | 503 | 627 | **1,008** |
| L2 | 91 | 541 | 85 | **613** |
| L2G | 5 | 23 | 153 | **167** |
| **`QudJP.Tests` 計** | **148** | **1,067** | **865** | **1,788** |

> **注**: `L2/` 配下に **37 メソッド** が `[Category("L2")]` 未付与のまま存在 (例: `AbilityManagerLineTranslationPatchTests.cs`)。`dotnet test --filter TestCategory=L2` だけだと L2/ 全件は拾われない。実態は **L1 503 / L2 504 (categorized) / L2G 23 / 未カテゴリ 37 = 1,067** メソッド。

#### `QudJP.Analyzers.Tests`

| ファイル | テストメソッド |
|---|---:|
| `FallbackLoggingAnalyzerTests.cs` | |
| `TraceLogPrefixAnalyzerTests.cs` | |
| `HarmonyPatchTryCatchAnalyzerTests.cs` | |
| **計 (3 files)** | **9** |

> **注**: 現状 CI (`.github/workflows/ci.yml`) では `QudJP.Tests` のみが build + test され、`QudJP.Analyzers.Tests` は CI に入っていない。ローカル `dotnet test Mods/QudJP/Assemblies/QudJP.Analyzers.Tests/...` で個別に実行可能。
...
`scripts/tests/` 配下に、`validate_xml.py` / `diff_localization.py` / `build_release.py` / `triage_*.py` / `scanner_*.py` / `translate_corpus_batch.py` / `verify_inventory.py` / `reconcile_inventory_status.py` 等の Python ツール各種に対する unit + integration テスト一式。

### 全体合計

| 区分 | メソッド数 | invocation 数 |
|---|---:|---:|
| C# `QudJP.Tests` | **1,067** | **1,788** |
| C# `QudJP.Analyzers.Tests` | **9** | **9** |
| Python pytest | **193** | **193** |
| **合計** | **1,269** | **1,990** |
