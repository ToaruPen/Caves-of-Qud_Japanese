...
- **PR1 AST subset (required):**
  ...
  - HSE marker expansion: `LiteralPercent + identifier + LiteralPercent` triples (e.g. `"In %" + year + "%, ..."`) are rewritten to mirror `HistoricStringExpander`'s runtime substitution before the regex is anchored. Slot type changes from the default `unresolved-local` to `hse-expansion`. Issue #423 ships a single-entry HSE table for `year`. The runtime helper `XRL.Annals.QudHistoryHelpers.ConvertYearToSultanateCalendarEra(year, endOfSultanateDate)` returns `<digits> BR` when the event predates the sultanate flipYear and `<digits> AR` otherwise, so the extractor emits `(.+?)\ (?:BR|AR)` (capture group keeps the digits; era is discarded — current Japanese template `{0}年` does not preserve era). The `sample_source` field shows the AR form for human readability; the `extracted_pattern` regex carries the alternation. Other HSE variables are deferred to PR2+ if they appear in the remaining 30 Annals files.
