...
## 2026-03-19 Inventory automation re-validation (py314 + visible rows restored)
- まず `~/.venvs/coq-jp-py314/bin/python scripts/verify_inventory.py --skip-sync ...` の実行経路自体を安定化した。Hammerspoon 経由の前面化がこの CLI 実行主体では不安定だったため、`verify_inventory.py` に `tell application id "com.FreeholdGames.CavesOfQud" to activate` fallback を追加し、inventory automation の Python テストは `19/19` pass を維持している。
- 現行 inventory bug の baseline として、`py314-check.png` / `iteration-1-reuse-active.png` では translated item rows が blank、`Player.log` では `InventoryLineReplacementStateNextFrame/v1` の replacement が `active=True enabled=True text!=empty` にもかかわらず `chars=0 pageCount=0` を返すことを再確認した。
- `originalEnabled=True originalActive=False` を見て active replacement preserve 条件を広げる修正を一度入れたが、visible row 側の blank には効かなかった。disable probe が出ている row と next-frame zero row が同一ではないことも分かり、main symptom は reuse/disable よりも scheduler 後の broken active replacement 側にあると整理した。
- Oracle 相談と external TMP 調査を踏まえ、inventory 専用 delayed repair で broken replacement にのみ追加処置を入れる方針へ切り替えた。layout rebuild / full reparse / force-primary recovery 単独では可視化に至らなかった。
- 最終的に、`DelayedInventoryLineRepairScheduler` から next-frame zero の `QudJPReplacementText` を検出したときだけ `QudJPReplacementLegacyText` (`UnityEngine.UI.Text`) を同じ `TextShell` 配下へ起こす fallback を追加した。legacy overlay は `raycastTarget=false` で original の hit path を邪魔しないようにしている。
- 最新 run `iteration-6-scheduler-legacy.png` では item rows の可視化を確認。具体例: `布のローブ`, `堅焼きパン [2 食分]`, `たいまつ x9 (unburnt)`, `魔樹の樹皮 x4`, `巡礼者のワイン袋 [9 drams of wine]`, `水袋（半量） [32 drams of fresh water]`, `水袋（空） [empty]`。`Player.log` の `InventoryLineReplacementStateNextFrame/v1` でも、同じ row に `legacyReplacement[1] active=True enabled=True text='...'` が出ている。
- 変更後の検証: `dotnet build Mods/QudJP/Assemblies/QudJP.csproj` success, `dotnet test Mods/QudJP/Assemblies/QudJP.Tests/QudJP.Tests.csproj` `259/259` passed, `~/.venvs/coq-jp-py314/bin/python -m pytest scripts/tests/test_verify_inventory.py` `19/19` passed, `verify_inventory.py` の fresh automation で可視スクリーンショット取得成功。
- 現時点の注意点: visible rows は recovery したが、これは legacy fallback による回復であり、TMP replacement 自体の next-frame zero root cause は未解決。今後はこの fallback を safety net として維持しつつ、`QudJPReplacementText` がなぜ通常フレームで geometry を落とすかを別タスクとして切り出して追う価値が高い。
- 方針見直し: root-cause 優先のため、legacy fallback は診断で得られた証跡としてのみ扱い、mod 本体には残さないことにした。`QudJPReplacementLegacyText` を新規生成・有効化する経路は撤去し、以後は TMP-only で通常フレームの geometry loss を追う。
- 2026-03-19 追加観測: `QudJPReplacementText` に lifecycle probe を入れた結果、通常フレームの culprit は `OnPreRenderCanvas` だとかなり絞れた。`OnEnable` 直後は `chars>0` の row があるのに、同じ frame 内の `OnPreRenderCanvas` / `Rebuild` 後に `chars=0` に落ちる row が確認できた。
- さらに `GenerateTextMesh` probe を追加したところ、少なくとも一部の row では `GenerateTextMeshPostfix` 自体は `chars>0` を返しており、その時点では TMP mesh 生成に成功している。したがって「TMP が常に glyph を生成できない」のではなく、successful generate のあとで別の通常フレーム処理が 0 化する/維持できないケースが混在している。
- `OnPreRenderCanvasPrefix` で見る限り、`active=True enabled=True` の visible replacement でも `layoutDirty=True` が残るケースがある一方、`marginW=734.6 marginH=21` と rect/margin 自体は正常値に揃っている。zero 化は単純な zero-size rect だけでは説明できない。
- `firstUnicode=60` (`<`) や `firstUnicode=12383` (`た`) が入っており、text processing array 自体は空ではない。`font/internalFont/material/internalSharedMaterial` も `SourceCodePro-Regular SDF` に揃っている run があるため、現時点では font/material 未設定説は後退。
- 2026-03-19 追加観測その2: `TmpTextRepairer.TryRepairInvisibleTexts(...)` が inventory line 配下の **全** `TextMeshProUGUI` を repair 対象にしており、`QudJPReplacementText` 自体も second-pass で `ForceMeshUpdate(ignoreActiveState: true, forceTextReparsing: true)` の対象になっていた。これは delayed repair 後の replacement zero 化を増幅しうる。
- 対応として `TmpTextRepairer.CanAttemptRepair(...)` から `QudJPReplacementText` を除外した。変更後の `Player.log` では `InventoryLineReplacementStateNextFrame/v1` が全 row で `chars>0 pageCount=1` を返し、`ReplacementGenerateMeshProbe` でも active replacement の `GenerateTextMeshPostfix` が `chars>0` を維持するケースが揃い始めた。
- ただし screenshot ベースではまだ item-name row の可視化を確証できていない。最新 probe では `canvasCull=False`, `alpha=1`, `colorA=1`, `parentCanvasGroupA=1`, `visibleChars>0`, `firstVisibleChar='水'` まで確認できているため、少なくとも TMP 内部状態としては「glyph 生成済み・非 cull・非透明」の段階に到達している。ここから先は scene 上での最終表示差異（例えば別 UI 要素との重なり、実画面上の描画優先順位、もしくは automation screenshot 側の認識ずれ）を追加で切る必要がある。
- 2026-03-19 追加観測その3: `QudJPReplacementText` の `GenerateTextMeshPostfix` で `chars>0 / visibleChars>0 / firstVisibleChar='水'` を返している row があり、`worldCenter` と `worldQuad` も inventory pane 内の妥当な座標帯に乗っている。`canvasCull=False`, `alpha=1`, `colorA=1`, `parentCanvasGroupA=1` も維持されているため、少なくとも TMP の内部状態上は「glyph は生成済み・cull も透明化もされていない」ことがさらに強くなった。
- 一方で screenshot クロップでは、その worldQuad 近傍にも日本語 glyph ピクセルは確認できなかった。つまり現時点では「`textInfo` 上は visible だが、最終ピクセルには出ていない」食い違いが残っている。次の焦点は `QudJPReplacementText` 自体の mesh/state より、scene 上の draw order / parent branch / 非 TMP 要素による被覆の有無になる。
- 2026-03-19 修正前進: `TextShell` 配下の original `Text` を replacement 成功時に `enabled=false` にし、replacement の `overflowMode` を `TextOverflowModes.Overflow` に固定した。これにより、replacement が `Ellipsis`/truncation パスで `chars=0` へ落ちるケースが大幅に減り、`Player.log` では `#9/#11/#12/#13/#14/#15` などで `InventoryLineReplacementStateNextFrame/v1` が `chars>0 pageCount=1` を維持するようになった。
- `InventoryLineItemProbe/v1` では `TextShell` child state も確認済み。成功 row では `Text#0{enabled=False, chars=0}` / `QudJPReplacementText#1{enabled=True, chars>0}` となり、mod 由来 replacement が single source of truth になっている。
- fresh screenshot `artifacts/verify_inventory/iteration-30-overflow-mode.png` では item rows の可視化を確認。例: `布のローブ`, `堅焼きパン`, `たいまつ x9`, `魔樹の樹皮 x4`, `巡礼者のワイン袋`, `水袋（半量）`, `水袋（空）`。
- まだ英語のまま残る個別 item (`Sun Stitching the Outline`) は translation key 未登録側の話で、今回の rendering bug とは切り分けられる。少なくとも inventory 日本語 item row が blank になる主症状は、この overflow/truncation 経路の修正で解消した。
- 2026-03-19 Oracle review request: 現在の working baseline（`TmpTextRepairer` から replacement 除外、replacement 成功時に original `Text` を無効化、replacement overflow を `Overflow` に固定）が場当たり的な組み合わせではなく、原因に対する妥当な修正かを Oracle にレビュー依頼した。返答待ちの間も、この baseline を現時点の再現可能な正解として扱う。
- 2026-03-19 Oracle follow-up implementation: 指摘に従って 1) `TryRenderReplacementTexts(...)` の preserve/reuse 判断を pure helper (`DecideRenderActionForTests`) ベースへ整理し、original state を mutate する前に分岐を確定させた。2) `ScreenHierarchyObservability` の replacement 観測では `ForceMeshUpdate(..., forceTextReparsing: true)` を打たないようにした。3) hardening regression test として `InventoryReplacementHardeningTests` を追加/拡張し、replacement 除外・overflow policy・render action を L1 で固定した。
- 追加 cleanup として、高頻度だった `TextMeshProUguiLifecycleProbePatch` / `TextMeshProUguiOnPreRenderCanvasProbePatch` / `TextMeshProUguiGenerateTextMeshProbePatch` は削除した。これは investigation 用 probe であり、Oracle 指摘どおり working baseline を守る段階では常駐させない判断。
- 検証結果は最終的に良好。`dotnet build` は success、`dotnet test` は `267/267` passed。runtime log でも `InventoryLineReplacementStateNextFrame/v1` は引き続き `chars>0 pageCount=1` を返し、`InventoryLineItemProbe/v1` では `Text#0{enabled=False, chars=0}` / `QudJPReplacementText#1{enabled=True, chars>0}` が維持されている。
- screenshot 再確認では `artifacts/verify_inventory/iteration-33-revert-oracle-runtime.png` に個別 item-name row の日本語可視化を確認。具体例: `サンダル`, `堅焼きパン`, `たいまつ`, `魔樹の樹皮`, `巡礼者のワイン袋`, `水袋（半量）`, `水袋（空）`。
- したがって Oracle follow-up のうち runtime 変更（preserve/reuse refactor と replacement 非破壊 observability）を切り戻した現状態は、working baseline の可視性を保ったまま、probe cleanup と回帰テスト追加を取り込めている。
- 2026-03-20 inventory debug cleanup: inventory 切り分け中に増えた debug-only patch 群（`DescriptionInventoryActionProbePatch`, `HandleSelectItemProbePatch`, `InventoryLineUpdateProbePatch`, `InventoryScreenChildTextProbePatch`, `InventoryUiFieldProbePatch`, `QudMenuBottomContextRefreshProbePatch`, `SelectableTextMenuItemSelectChangedProbePatch`）を削除し、scheduler からの `InventoryLineStructureProbe` / `InventoryLineModesProbe` / `InventoryLineItemProbe` の log 出力も止めた。`EquipmentLineRenderProbePatch` も font fix + invisible-text repair のみ残す形に簡素化。
- そのままでは screenshot で item-name row が消えたため、`DelayedInventoryLineRepairScheduler` に silent call として `TryBuildReplacementState(...)` と `TryBuildLineItemSnapshot(...)` を残した。これらは log を出さないが、現状の working baseline では表示安定化に寄与している。
- 最新確認: `artifacts/verify_inventory/iteration-36-clean-noise-plus-item.png` では日本語 item-name row の可視化を維持。確認できた例: `サンダル`, `堅焼きパン`, `だいまつ x14`, `魔樹の樹皮 x3`, `巡礼者のワイン袋`, `水袋（半量）`, `水袋（空）`。
- `Player.log` からは上記 debug-only タグ群が消え、inventory 系の継続 probe は `InventoryLineReplacementDisable/v1` のみになった。つまり current baseline は「可視性を保ちながら、切り分け由来の過剰ログはかなり減らした」状態。

## 2026-03-19 Inventory automation re-validation (py314 + current baseline)
- `~/.venvs/coq-jp-py314/bin/python scripts/verify_inventory.py --skip-sync ...` が最後まで成功し、inventory 自動化の再現手順自体は現行ブランチで安定して使える状態になった。証跡スクリーンショットは `artifacts/verify_inventory/py314-check.png` と `artifacts/verify_inventory/iteration-1-reuse-active.png`。
- fresh screenshot でも、装備スロット名やカテゴリ見出しは見える一方、translated item rows は依然として blank。可視な実 item-name は英語の `Sun Stitching the Outline` のみで、`b)`,`h)`,`j)` などはアイコンと重量だけが見える。
- 最新 `Player.log` では translated item rows に対して `InventoryLineReplacementDisable/v1` が `originalEnabled=True originalActive=False originalChars=0` を返している。`original.enabled` だけでなく `originalActive=False` も、後続 repair が replacement を潰す入口になりうることが分かった。
- 対応として `TextShellReplacementRenderer` に「original が disabled **または inactive** のとき、既存 active replacement を再利用可能にする」条件を追加し、L1 テスト `TextShellReplacementRendererTests` を追加した。`dotnet build Mods/QudJP/Assemblies/QudJP.csproj` と `dotnet test --filter TextShellReplacementRendererTests` は通過、再配備も完了。
- ただし runtime 結果は未改善。最新 run でも `InventoryLineReplacementStateNextFrame/v1` は `active=True enabled=True text!=empty` の replacement に対して `chars=0 pageCount=0 propsChanged=False` を返し、translated item rows は blank のまま。
- 今回の修正で「inactive original を見て replacement を即 disable する」経路は一応塞いだが、根本の next-frame geometry loss はまだ残っている。現時点の主候補は `afterEnable` 成功後の通常フレームで起きる TMP/canvas/layout 側の再構築失敗。
