...
昇格基準は [`glossary-policy.md`](glossary-policy.md) にあります。
...
CI は `Mods/QudJP/Localization/` の変更に対して release-note fragment、translation-token、glossary consistency を自動検証します。

XML / JSON の構文、エンコーディング、L3 表示確認は CI では完結しません。編集後は必要に応じて手動で以下を実行してください:
...
- `scripts/tools/` 変更: `scripts/tools/**/*.csproj` を列挙し、見つかった tool projects をすべて Release build します。
